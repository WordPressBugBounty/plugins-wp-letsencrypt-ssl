...
require_once __DIR__ . '/le_admin_page_wrapper.php';
require_once dirname(__DIR__) . '/classes/le-advanced-scanner.php';
require_once dirname(__DIR__) . '/classes/le-security.php';
...
if (!class_exists('WPLE_SubAdmin')) {
    ...
    class WPLE_SubAdmin extends WPLE_Admin_Page
    {
        ...
        public function __construct()
        {
            ...
            add_action('wp_ajax_wple_update_security', [$this, 'wple_update_security']); //7.0.0
        ...
        public function wple_register_admin_pages()
        {
            ...
            add_submenu_page('wp_encryption', 'SSL Health and Headers', __('SSL Health and Headers', 'wp-letsencrypt-ssl') . ' ' . $notifications . '', 'manage_options', 'wp_encryption_ssl_health', [$this, 'wple_sslhealth_page']);

            add_submenu_page('wp_encryption', 'Advanced Security and Scanner', __('Advanced Security and Scanner', 'wp-letsencrypt-ssl'), 'manage_options', 'wp_encryption_security', [$this, 'wple_security_page']);


            add_submenu_page('wp_encryption', 'Login Security - Passkeys', __('Login Security - Passkeys', 'wp-letsencrypt-ssl'), 'manage_options', 'wp_encryption_login_security', [$this, 'wple_login_security_page']);
            ...
            add_submenu_page('wp_encryption', 'Mixed Content Scanner', __('Mixed Content Scanner', 'wp-letsencrypt-ssl'), 'manage_options', 'wp_encryption_mixed_scanner', [$this, 'wple_mixed_scanner_page']);
            //}

            if (wple_fs()->is__premium_only()) {
                //since 7.8.2
                add_submenu_page('options.php', 'Setup Wizard', __('Setup Wizard', 'wp-letsencrypt-ssl'), 'manage_options', 'wp_encryption_setup_wizard', [$this, 'wple_setup_wizard_page__premium_only']);
            }

            add_submenu_page('options.php', 'Debug log', __('Debug log', 'wp-letsencrypt-ssl'), 'manage_options', 'wp_encryption_log', [$this, 'wple_debug_log_page']);

            add_submenu_page('options.php', 'Malware & Integrity Report', __('Malware & Integrity Report', 'wp-letsencrypt-ssl'), 'manage_options', 'wp_encryption_malware_scan', [$this, 'wple_malwarescan_page']);
        ...
        {
            add_submenu_page('options.php', 'How-To Videos', __('How-To Videos', 'wp-letsencrypt-ssl'), 'manage_options', 'wp_encryption_howto_videos', [$this, 'wple_howto_page']);
            add_submenu_page('options.php', 'FAQ', __('FAQ', 'wp-letsencrypt-ssl'), 'manage_options', 'wp_encryption_faq', [$this, 'wple_faq_page']);
            ...
            if (is_array($submenu) && array_key_exists('wp_encryption', $submenu)) {
                foreach ($submenu['wp_encryption'] as $key => $val) {
                    if (in_array('wp_encryption_upgrade', $val)) {
                        ...
                        ///$submenu['wp_encryption'][$key][2] = 'https://wpencryption.com/?utm_source=wordpress&utm_medium=upgradepro&utm_campaign=wpencryption#pricing'; //medium=admin in june
                    ...
                    } else if (in_array('wp_encryption_login_security', $val)) {

                        //$submenu['wp_encryption'][$key][0] = '<span style="color:#adff2f">' . esc_html($submenu['wp_encryption'][$key][0]) . '</span>';
                        ///$submenu['wp_encryption'][$key][2] = 'https://wpencryption.com/?utm_source=wordpress&utm_medium=loginsecurity&utm_campaign=wpencryption';
      ...
      <button type="submit" name="wple_ssl">' . esc_html__('Save', 'wp-letsencrypt-ssl') . '</button>
        ...
        public function wple_force_https_handler()
        {
            ...
            if (isset($_POST['site-force-ssl'])) {
                ...
                //reverse proxy / loadbalancer detection
                $reverseproxy = false;
                ...
                if (!$client && $leopts['force_ssl'] != 0 && !is_ssl() && !$reverseproxy) {
                ...
                if ($leopts['force_ssl'] != 0) {
                    ...
                    if (!isset($_SERVER['HTTP_CF_VISITOR'])) { //avoid redirection loop with cloudflare flexible ssl
                    ...
                    if ($leopts['force_ssl'] == 1) { //wordpress redirect
                        if ($prevforce == 2) {
                            $this->wple_clean_htaccess();
                        }

                        ///WPLE_Trait::wple_send_reverter_secret($reverter);
                    } elseif ($leopts['force_ssl'] == 2) { //htaccess redirect
            ...
            if (isset($_GET['forceenablehttps'])) {
                ...
                if ($_GET['forcetype'] == 1) { //wp redirect method
    ...
    <h4>' . esc_html__('Does installing the plugin will instantly turn my site https?', 'wp-letsencrypt-ssl') . '</h4>
      ...
      <p>' . WPLE_Trait::wple_kses(sprintf(
                __('Please use our <a href="%s" target="%s">Plugin support forum</a>. <b>PRO</b> users can register free account & use priority support at support.wpencryption.com. More info - https://wpencryption.com', 'wp-letsencrypt-ssl'),
    ...
    <span class="toggle-label">' . $emailattachment . '</span>
            ...
            if (wple_fs()->is__premium_only()) {
                $spmodedisabled = get_option('wple_disable_spmode');
                $emailcertswitch .= '<div class="plan-toggler" style="text-align: left; margin: 40px 0 0px;"><span></span><label class="toggle">
      <input class="toggle-checkbox disable-spmode-switch" type="checkbox" ' . checked($spmodedisabled, true, false) . '>
      <div class="toggle-switch" style="transform: scale(0.9);"></div>
      <span class="toggle-label">Disable Alternative SSL Solution (<small>Please leave this disabled unless our support team request to enable</small>)</span>
      </label>
      </div>';

                $spmodeforced = get_option('wple_force_spmode');
                $emailcertswitch .= '<div class="plan-toggler" style="text-align: left; margin: 40px 0 0px;"><span></span><label class="toggle">
      <input class="toggle-checkbox force-spmode-switch" type="checkbox" ' . checked($spmodeforced, true, false) . '>
      <div class="toggle-switch" style="transform: scale(0.9);"></div>
      <span class="toggle-label">Force Enable Alternative SSL Solution (<small>Please leave this disabled unless our support team request to enable</small>)</span>
      </label>
      </div>';
            }
    ...
    <button class="wple-scan" style="margin-top:40px" data-nc="' . wp_create_nonce('wplemixedscanner') . '">' . esc_html__('START THE SCAN', 'wp-letsencrypt-ssl') . '</button>
    </div>';
    ...
    // }
        ...
        public function wple_email_certs_setting()
        {
            ...
            if (wple_fs()->is__premium_only()) {
                $val = ($_POST['spmode'] == 'true') ? true : false;
                update_option('wple_disable_spmode', $val);

                $val = ($_POST['forcespmode'] == 'true') ? true : false;
                update_option('wple_force_spmode', $val);
            }
        ...
        private function wple_ssl_score()
        {
            ...
            $viaAlternateMethod = array(); // Enabled through secondary sources
            ...
            set_transient('wple_alternate_headers', $viaAlternateMethod, 60 * 60); //1 hour
    ...
    <div class="wple-scorebar"><span data-width="' . (int) $score . '" style="width:' . (int) $score . '%;background:#' . esc_attr($scorecolor) . '"></span></div>';
            ...
            if ($score == 70) $output .= '<h3 class="score-prom" style="margin-bottom:30px">You still have major task pending!</h3>';
    ...
    <h2>Active SSL Info</h2>';
            ...
            $sslopts = array(
                'Enable Mixed Content Fixer' => ['key' => 'mixed_content_fixer', 'desc' => 'Fixes basic mixed content issues like images, urls, stylesheets, etc.,'],
                'Enable HttpOnly Secure Cookies' => ['key' => 'httponly_cookies', 'desc' => 'Cookies are made accessible server side only. Even if XSS flaw exists in client side or user accidently access a link exploting the flaw, client side script cannot read the cookies'],
                'Enable SSL Monitoring' => ['key' => 'ssl_monitoring', 'desc' => 'You will get automated email as well as dashboard notification when SSL is expiring within 10 days'],
            );

            $sec_headers = array(
                'Enable Upgrade Insecure Requests Header' => ['key' => 'upgrade_insecure', 'desc' => 'Upgrades insecure HTTP requests to HTTPS'],
                'Enable HSTS Strict Transport Header' => ['key' => 'hsts', 'desc' => 'HSTS Strict Transport blocks all insecure assets & resources which cannot be served over HTTPS'],
                'Enable X-XSS Header' => ['key' => 'xxss', 'desc' => 'Blocks page loading when cross site scripting attacks are detected'],
                'Enable X-Content-Type-Options Header' => ['key' => 'xcontenttype', 'desc' => 'Protects against MIME sniffing vulnerabilities'],
                'Enable X-Frame-Options Header (Premium)' => ['key' => 'xframe', 'desc' => 'Blocks embedding of your site on other domains to avoid click-jacking attacks', 'premium' => 1],
                'Enable Referrer-Policy Header (Premium)' => ['key' => 'referrer', 'desc' => 'Blocks referrer info transfer when HTTPS to HTTP scheme downgrade happens', 'premium' => 1],
                //'Enable Important Security Headers' => ['key' => 'security_headers', 'desc' => 'Implements important security headers like X-XSS-Protection, Referrer-Policy, X-Content-Type-Options eliminating man in the middle attacks & improves site security'],
            );
    ...
    <ul>';
            ...
            foreach ($sec_headers as $optlabel => $optarr) {
                ...
                $isActive = get_option("wple_" . esc_attr($optarr['key']));
                $viaSecondary = false;
                $alreadyEnabled = '<span></span>';

                if (in_array($optarr['key'], $enabledViaAlternate)) {
                    $viaSecondary = 1;
                }

                //Not activated from wp-encryption but its already active
                if (!$isActive && $viaSecondary) {
                    $alreadyEnabled = '<span class="dashicons dashicons-info-outline wple-tooltip" style="margin: 7px 0; color: #ff8900;" data-tippy="We have detected that this header is already enforced via other sources like wp-config, htaccess or php.ini"></span>';
                }
    ...
    <li class="wple-sec-scanner"><a href="https://scan.wpencryption.com/?domain=' . esc_attr(site_url()) . '" target="_blank">Security Header Scanner <span class="dashicons dashicons-external"></span></a></li>';
        ...
        public function wple_update_settings()
        {
            ...
            $allowed = array('mixed_content_fixer', 'hsts', 'security_headers', 'upgrade_insecure', 'httponly_cookies', 'ssl_monitoring', 'xxss', 'xcontenttype', 'xframe', 'referrer', 'vulnerability_scan', 'daily_vulnerability_scan', 'notify_vulnerability_scan', 'autofix_vulnerability_scan', 'notify_malware_scan', 'daily_malware_scan');
            ...
            $score_exclusions = array('upgrade_insecure', 'ssl_monitoring', 'vulnerability_scan', 'daily_vulnerability_scan', 'notify_vulnerability_scan', 'autofix_vulnerability_scan');

            $no_writing = array('ssl_monitoring', 'vulnerability_scan', 'daily_vulnerability_scan', 'notify_vulnerability_scan', 'autofix_vulnerability_scan'); //file writing not required

            $ssl_not_required = array('httponly_cookies', 'ssl_monitoring', 'xxss', 'xcontenttype', 'xframe', 'referrer', 'vulnerability_scan', 'daily_vulnerability_scan', 'notify_vulnerability_scan', 'autofix_vulnerability_scan');
            ...
            if ($val == 0) {
                ...
                if ($opt == 'xxss' && $xctype_header) $out = 0;
                if ($opt == 'xcontenttype' && $xxss_header) $out = 0;
                ...
                if (false == get_option('wple_ssl_valid') && !in_array($opt, $ssl_not_required)) { //no valid ssl detected
                    $out = 1;
                    echo esc_html($out);
                    exit();
                }
        ...
        private function wple_addremove_security_headers(&$out, $opt, $val)
        {
            if ($opt == 'xxss' || $opt == 'xcontenttype' || $opt == 'xframe' || $opt == 'referrer' || $opt == 'upgrade_insecure' || $opt == 'hsts') {
                ...
                if ($val == 1) { //add request
                    if (is_writable(ABSPATH . '.htaccess')) {

                        WPLE_Trait::wple_clean_security_headers(); //complete block
    ...
    <th>Reference</th>';
        ...
        public function wple_enforce_security_headers($headers = array())
        {
            ...
            if (wple_fs()->can_use_premium_code__premium_only()) {
                ...
                if (get_option('wple_xframe')) {
                    $headers['X-Frame-Options']  = 'SAMEORIGIN';
                }

                if (get_option('wple_referrer')) {
        ...
        public function wple_security_page()
        {
            ...
            $vuln_headers = array(
                'Enable Daily Scan (Premium)' => ['key' => 'daily_malware_scan', 'desc' => 'Automated daily scan of entire WordPress directory for malware & suspicious files.', 'premium' => 1],
                'Enable Instant Notification (Premium)' => ['key' => 'notify_malware_scan', 'desc' => 'Immediately notifies you via email / admin notice when a malware is detected on site.', 'premium' => 1]
            );
            ...
            <div id="wple-vulnerability-scanner" class="wple-malware-scanner">
    ...
    <ul>';
            ...
            $vuln_headers = array(
                'Enable Vulnerability Scanner' => ['key' => 'vulnerability_scan', 'desc' => 'Scans installed versions of your WordPress core, plugins, themes for known vulnerabilities.', 'premium' => 0],
                'Enable Daily Scan (Premium)' => ['key' => 'daily_vulnerability_scan', 'desc' => 'Automatically Scans installed versions of your WordPress core, plugins, themes for known vulnerabilities everyday.', 'premium' => 1],
                'Enable Instant Notification (Premium)' => ['key' => 'notify_vulnerability_scan', 'desc' => 'Immediately notifies you via email / admin notice when a medium+ vulnerability is found.', 'premium' => 1],
                'Enable Automatic Vulnerability Fixing (Premium)' => ['key' => 'autofix_vulnerability_scan', 'desc' => 'Automatically update vulnerable plugin, theme, core as soon as updated patch is found', 'premium' => 1]
            );
    ...
    ' . $this->wple_security_settings() . '
            ...
            $security_actions = array(
                array(
                    'Rename default database prefix of wp_',
                    'WordPress database tables use wp_ prefix by default. Rename it to random prefix to further enhance database security.',
                    'rename_db_prefix'
                ),
                array(
                    'User with username "admin" exists on site',
                    'Having "admin" user on site is first target for attackers to perform bruteforce / password guessing attacks. Rename username to something else.',
                    'rename_admin'
                ),
                array(
                    'One of the administrator have same username & display name',
                    'While having username as display name, Attackers already know your login username and can perform bruteforce / password guessing attacks. Edit profile to change display name different than login username.',
                    'rename_displayname'
                )
            );
    ...
    <ul>';
      ...
      <div id="wple-upgradepro">';
        ...
        public function wple_setup_wizard_page__premium_only()
        {
         ...
         * @return void
         */
        public function wple_login_security_page()
        {
            $html = '<div class="wple-login-security">';

            $wplepasskey = get_option('wple_passkeys');
            $actions_ul = '<ul><li>
        <div class="plan-toggler" style="text-align: left; margin: 40px 0 0px;"><span></span><label class="toggle" style="width:95%;line-height:1.4em;">
        <input class="toggle-checkbox wple-passkey-feature" type="checkbox" ' . checked($wplepasskey, true, false) . '>
        <div class="toggle-switch" style="transform: scale(0.85);"></div>
        <span class="toggle-label">
        <strong>Enable passwordless secure login using passkeys</strong><br />
        <small>Passkeys are not a two-factor authentication (2FA) method. They are a passwordless multi-factor authentication (MFA) solution that allows users to securely log in without using a password - No passwords. No brute force. Just secure, frictionless login with passkey-based WebAuthn - powered by your browser and protected by fingerprint, Face ID, or device PIN. This will enable new passkey registration option on edit profile page and login via passkey feature.</small></span>
        <br />
        <br />
        <strong>Note:</strong> In the Free edition of WP Encryption, each user can register only one Passkey. The <a href="https://wpencryption.com/?utm_source=wordpress&utm_medium=loginsecurity&utm_campaign=wpencryption" target="_blank">Premium</a> edition enables users to register unlimited Passkeys, allow passkey only login and gives you the option to enforce passkeys for specific user roles (Ex: administrators).
        </label>
        </div>
        </li>
        </ul>';
            ...
            $html .= '<div class="wple-ssl-settings wple-loginsecurity">
        <div style="text-align: center; padding: 20px;"><img src="' . esc_attr(WPLE_URL) . '/admin/assets/wple-login.png" style="max-width:250px"/></div>        
        <div id="wple-sec-actions">
        ' . $actions_ul . '    
        </div>
            <div class="wple-login-steps">
            <span><img src="' . esc_attr(WPLE_URL) . '/admin/assets/register-passkey.png"/><h3>Register Passkey</h3><p>Register new passkey via your profile page. All users including subscribers can register their passkey from profile page.</p></span>
            <span><img src="' . esc_attr(WPLE_URL) . '/admin/assets/save-passkey.png"/><h3>Save Passkey</h3><p>You can store the passkey on your password manager, pen drive or phone / iPad protected by fingerprint or password,</p></span>
            <span><img src="' . esc_attr(WPLE_URL) . '/admin/assets/login-passkey.png"/><h3>Login via Passkey</h3><p>Login via registered passkey easily without any password. When logging in from desktop, you can easily scan QR code from your phone / iPad if passkey is stored on your device.</p></span>
            </div>
        </div>';
