...
class WPLE_Handler
{
    ...
    public function __construct()
    {
        ...
        if (wple_fs()->can_use_premium_code__premium_only()) {
            ...
            add_action('wp_ajax_wple_wizard_generatessl', [$this, 'wple_wizard_generatessl__premium_only']);
        }
    ...
    public function wple_auto_handler()
    {
        if (isset($_GET['wpleauto'])) {
            if (get_option('wple_order_refreshed')) delete_option('wple_order_refreshed');

            $leopts = get_option('wple_opts');
            new WPLE_Core($leopts); //continue verification
        }
    }
    ...
    private function primary_ssl_install_request()
    {
        ...
        if (isset($_POST['generate-certs'])) {
            ...
            if (wple_fs()->can_use_premium_code__premium_only()) {
                if (is_multisite()) {
                    ...
                    if (isset($_POST['wple_domain']) && !empty($_POST['wple_domain'])) {
                        $leopts['domain'] = sanitize_text_field($_POST['wple_domain']); //mapped domain
                    }
                }
            }

            if (isset($_POST['wple_domain']) && !is_multisite()) {
                $leopts['subdir'] = 1; //flag domain as primary domain of subdir site
        ...
        //wildcard ssl
        if (wple_fs()->can_use_premium_code__premium_only()) {
            if (isset($_POST['generate-wc-certs'])) {
                if (!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['letsencryptwildcard'])), 'legeneratewildcard') || !current_user_can('manage_options')) {
                    die('Unauthorized request');
                }
                ...
                new WPLE_Core($leopts, true, true);
            }
        }
    }
    ...
    public function wple_intro_pricing_handler()
    {
        ...
        if (isset($_GET['gofree'])) {
            ...
            exit();
        } else if (isset($_GET['gopro'])) {
            ///set_transient('wple_plan_chosen', true, 15 * DAY_IN_SECONDS);
            update_option('wple_plan_choose', 1);
            if ($_GET['gopro'] == 2) { //unlimited
    ...
    public function wple_run_vulnerability_scan()
    {
        ...
        if (wple_fs()->can_use_premium_code__premium_only()) {
            if (get_option('notify_vulnerability_scan')) {
                $this->wple_send_vuln_notification_email($threats);
            }
        }
    }
    ...
    public function wple_wizard_generatessl__premium_only()
    {
        ...
        WPLE_Trait::wple_logger("Wizard: Generating SSL\n", 'success', 'a', false);
        ...
        $leopts['wizard'] = 1; //flag for wizard
