    ...
    class WPLE_Security
    {
        ...
        public function wple_disable_x_poweredby_header($enable = true)
        {
            if (!is_writable(ABSPATH . '.htaccess')) {
                echo 0; //alert Could not update setting! Please try again.
                exit();
            }

            if ($enable) { //add request
                if (is_writable(ABSPATH . '.htaccess')) {
        ...
        public static function wple_security_score()
        {
            ...
            foreach ($scoredefinitions as $key => $desc) {
                ...
                if ($key == 'https_enforced') {
                ...
                } else if ($key == 'disable_register') {
                    if (!get_option('users_can_register')) $isenabled = true;
                } else if ($key == 'critical_issues') {
                    ...
                    if ($site_health !== false) {
                        ...
                        if ($critical_count == 0) $isenabled = true;
                    } else {
                        $isenabled = true;
                    }
                } else if ($key == 'disable_directory') {
                    if (in_array('disable_directory_listing', $security_opts)) {
                        $isenabled = true;
                    }
                } else if ($key == 'latest_vulnerability') {
                ...
                } else if ($key == 'daily_vulnerability_scan' || $key == 'daily_malware_scan') {
        ...
        public static function wple_feature_check($key)
        {
            ...
            switch ($key) {
                ...
                case 'advanced_security':
                    if (wple_fs()->can_use_premium_code__premium_only()) {
                        if (get_option('wple_xframe') && get_option('wple_referrer')) {
                            return 1;
                        }
                    }
                    break;
