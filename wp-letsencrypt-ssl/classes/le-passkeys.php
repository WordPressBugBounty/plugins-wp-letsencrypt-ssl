...
class WPLE_Passkeys
{
    ...
    public static function wple_ajax_remove_passkey()
    {
        ...
        $result = $credRepo->findOneByID($cred_id); // Check if credential exists and belongs to the user (optional, can be enforced in delete method)
...
?>
        ...
        <table class="wp-list-table widefat striped table-view-list wple-passkeys" width="100%">
            <thead>
                <tr>
                    <th><?php esc_html_e('Name', 'wp-letsencrypt-ssl'); ?></th>
                    <th><?php esc_html_e('Created At', 'wp-letsencrypt-ssl'); ?></th>
                    <th><?php esc_html_e('Last Used', 'wp-letsencrypt-ssl'); ?></th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                <?php if (empty($credentials)) {
                    ...
                    foreach ($credentials as $cred): ?>
                        <tr>
                            <td><?php echo esc_html($cred['device_name']); ?></td>
                            <td><?php echo esc_html($cred['created_at']); ?></td>
                            <td><?php echo esc_html($cred['last_used']); ?></td>
                            <td>
                                <button class="button wple-remove-passkey"><a href="<?php echo wp_nonce_url(admin_url('admin-ajax.php?action=wple_remove_passkey&cred_id=' . esc_attr($cred['id'])), 'wple-rempasskey'); ?>"><?php esc_html_e('Remove', 'wp-letsencrypt-ssl'); ?></a></button>
                            </td>
                        </tr>
                <?php endforeach;
                } ?>
    ...
    ?>
        <h2 id="wple-passkeys"><?php esc_html_e('WP Encryption - Passkeys', 'wp-letsencrypt-ssl'); ?></h2>
        <?php SELF::render_passkeys_table(); ?>
        <table class="form-table">
            <tr>
                <td>
                    <div id="wple-passkey-area">
                        <button id="wple-register-passkey" class="button"><?php esc_html_e('Register New Passkey', 'wp-letsencrypt-ssl'); ?></button>
                        <span id="wple-passkey-msg" style="margin-left:12px;line-height:30px;">
                            <?php if (isset($_GET['removed'])) { ?>
                                <i style="color:red;"><?php esc_html_e('Passkey removed successfully!', 'wp-letsencrypt-ssl'); ?></i>
                            <?php } ?>
                        </span>
                    </div>
                </td>
...
<?php
    ...
    public static function ajax_get_register_options()
    {
        ...
        $options = array(
            'challenge' => $challenge,
            'rp' => array('name' => get_bloginfo('name'), 'id' => wp_parse_url(home_url(), PHP_URL_HOST)),
            'user' => array(
    ...
    public static function ajax_register_passkey()
    {
        ...
        if (!$att_raw) {
            wp_send_json_error(array('message' => 'Invalid attestation object encoding.'));
        }

        $offset  = 0;
        $att_map = SELF::wple_cbor_decode($att_raw, $offset);
        if (! is_array($att_map) || ! isset($att_map['authData'])) {
            wp_send_json_error(array('message' => 'Invalid attestation object structure.'));
        }

        $auth = SELF::wple_parse_auth_data($att_map['authData']);
        if (! $auth) {
            wp_send_json_error(array('message' => 'Could not parse authenticator data.'));
        }

        if (! SELF::wple_verify_rp_id_hash($auth['rpIdHash'])) {
            wp_send_json_error(array('message' => 'RP ID mismatch.'));
        }

        if (! $auth['userPresent']) {
            wp_send_json_error(array('message' => 'User presence flag not set.'));
        }
        if (! $auth['credentialId'] || ! $auth['publicKeyCbor']) {
            wp_send_json_error(array('message' => 'No credential data in authenticator response.'));
        }

        $pem = SELF::wple_cose_to_pem($auth['publicKeyCbor']);
        if (! $pem) {
            wp_send_json_error(array('message' => 'Unsupported public key algorithm.'));
        }
        ...
        $credRepo = new Credential_Repository();
        $saved   = $credRepo->save_credential($uid, $cred_id, $auth['publicKeyCbor'], (int) $auth['signCount'], $device_name);

        if (! $saved) {
            wp_send_json_error(array('message' => 'Could not save credential (may already be registered).'));
        }

        wp_send_json_success(array('message' => 'Passkey registered!', 'credentialId' => $cred_id));
    ...
    {
        $raw = random_bytes(32);
        set_transient('wple_' . $type . '_tok_' . $token, SELF::wple_b64url_encode($raw), 9000);
    ...
    static function wple_challenge_get_token($type, $token)
    {
        ...
        $raw = get_transient($key); //b64 url encoded
    ...
    static function wple_ajax_login_options()
    {
        ...
        if ($user_login) {
            ...
            if ($u) {
                ...
                foreach ($credDB->get_user_credentials($lookup_user_id) as $c) {
                    $allow_creds[] = array('type' => 'public-key', 'id' => $c['credential_id']);
                }
            }
        }
        ...
        $options = array(
            ...
            'challenge'        => $challenge,
            'challengeToken'   => $token,   // sent back by JS in Step 2
            'timeout'          => 60000,
    ...
    static function wple_ajax_login_verify()
    {
        ...
        if (! $cred_id || ! $cdj || ! $auth_data || ! $sig || ! $token) {
            wp_send_json_error(array('message' => 'Missing assertion fields.'));
        }
        ...
        if (! $stored) {
            wp_send_json_error(array('message' => 'Unknown credential.'));
        }


        $raw_challenge = SELF::wple_challenge_get_token('login', $token);
        if (! $raw_challenge) {
            wp_send_json_error(array('message' => 'Challenge expired. Please try again.'));
        }

        if (! SELF::wple_verify_client_data($cdj, 'webauthn.get', $raw_challenge)) {
            wp_send_json_error(array('message' => 'Client data verification failed.'));
        }
        ...
        if (! $pem) {
            wp_send_json_error(array('message' => 'Stored public key is invalid.'));
        }

        $pub_key_res = openssl_get_publickey($pem);
        if (! $pub_key_res) {
            wp_send_json_error(array('message' => 'Could not load public key.'));
        }

        $signature = SELF::wple_b64url_decode($sig);
        $verified  = openssl_verify($signed_data, $signature, $pub_key_res, OPENSSL_ALGO_SHA256);
        if ($verified !== 1) {
            wp_send_json_error(array('message' => 'Signature verification failed.'));
        }

        $new_count = (int) $parsed['signCount'];
        if ($new_count !== 0 && $new_count <= (int) $stored['sign_count']) {
            wp_send_json_error(array('message' => 'Possible replay attack.'));
        }
        ...
        if (! $user) {
            wp_send_json_error(array('message' => 'Associated user not found.'));
        }
        ...
        wp_send_json_success(array('message' => 'Logged in!', 'redirect' => $redirect));
    }
    ...
    static function wple_parse_auth_data($auth_data)
    {
        ...
        $result = array(
            'rpIdHash'         => $rp_id_hash,
            'userPresent'      => (bool) ($flags_byte & 0x01),
            'userVerified'     => (bool) ($flags_byte & 0x04),
    ...
    static function wple_cbor_decode($data, &$offset)
    {
        ...
        if ($info < 24) {
            ...
            return null; // indefinite/reserved
        }
        ...
        switch ($major) {
            ...
            case 6: // semantic tag – decode wrapped value
    ...
    static function wple_ec_pem($x, $y)
    {
        ...
        $alg = "\x30" . SELF::wple_der_len(2 + strlen($oid_ec) + 2 + strlen($oid_p256))
            . "\x06" . SELF::wple_der_len(strlen($oid_ec))   . $oid_ec
            . "\x06" . SELF::wple_der_len(strlen($oid_p256)) . $oid_p256;

        $pt  = "\x04" . $x . $y;
        $bit = "\x03" . SELF::wple_der_len(strlen($pt) + 1) . "\x00" . $pt;

        $spki = "\x30" . SELF::wple_der_len(strlen($alg) + strlen($bit)) . $alg . $bit;
    ...
    static function wple_rsa_pem($n, $e)
    {
        ...
        $rsa = "\x30" . SELF::wple_der_len(2 + strlen($n) + 2 + strlen($e))
            . "\x02" . SELF::wple_der_len(strlen($n)) . $n
            . "\x02" . SELF::wple_der_len(strlen($e)) . $e;

        $oid = "\x2a\x86\x48\x86\xf7\x0d\x01\x01\x01";
        $alg = "\x30" . SELF::wple_der_len(2 + strlen($oid) + 2)
            . "\x06" . SELF::wple_der_len(strlen($oid)) . $oid
            . "\x05\x00";
